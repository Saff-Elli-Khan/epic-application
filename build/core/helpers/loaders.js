...
exports.LoadChildControllers =
  exports.LoadModules =
  exports.LoadLocalModules =
  exports.LoadModulesFromPlugins =
    void 0;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const common_1 = require("../../common");
const epic_odm_1 = require("@oridune/epic-odm");
const LoadModulesFromPlugins = (type) =>
  Object.keys(common_1.Configuration.plugins).reduce(
    (items, pluginName) => [
      ...items,
      ...(0, exports.LoadLocalModules)(type, {
        disabled: common_1.Configuration.plugins[pluginName].disabled,
        moduleDir: path_1.default.join(
          process.cwd(),
          `./node_modules/${pluginName}/build/${epic_odm_1.Utils.pluralize(
            type
          )}/`
        ),
      }),
...
const LoadLocalModules = (type, options) => {
  ...
  const ModuleDir =
    (options === null || options === void 0 ? void 0 : options.moduleDir) ||
    path_1.default.join(
      process.cwd(),
      `./${
        process.env.NODE_ENV === common_1.NODE_ENV.PRODUCTION ? "build" : "src"
      }/${epic_odm_1.Utils.pluralize(type)}/`
    );
  if (fs_1.default.existsSync(ModuleDir)) {
    // Get Lock Information
    const LockFilePath = path_1.default.join(ModuleDir, "../../epic.lock.json");
    const LockInfo = fs_1.default.existsSync(LockFilePath)
      ? require(LockFilePath)
      : {};
    const ModulesList = (LockInfo.modules || []).filter(
    ...
    return !((_a =
      ...
      null && _a !== void 0
      ? _a
      : common_1.Configuration.disabled)
      ? fs_1.default
          ...
          .map((filename) =>
            Promise.resolve()
              .then(() =>
                __importStar(require(path_1.default.join(ModuleDir, filename)))
              )
              .then(
                (module) =>
                  module[
...
const LoadChildControllers = (path, parentName) =>
  fs_1.default
    .readdirSync(path)
    .filter((filename) =>
    ...
    .map((filename) =>
      Promise.resolve()
        .then(() => __importStar(require(path_1.default.join(path, filename))))
        .then(
          (module) =>
            module[
