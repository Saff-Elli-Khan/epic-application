...
exports.DefaultCorsConfiguration = {
    origin: (_a = process.env.CORS_ALLOW_ORIGIN) === null || _a === void 0 ? void 0 : _a.split(",").map((item) => item.trim()),
    allowedHeaders: (_b = process.env.CORS_ALLOW_HEADERS) === null || _b === void 0 ? void 0 : _b.split(",").map((item) => item.trim()),
    credentials: ["true", "1"].includes(process.env.CORS_ALLOW_CREDENTIALS || ""),
    exposedHeaders: (_c = process.env.CORS_EXPOSED_HEADERS) === null || _c === void 0 ? void 0 : _c.split(",").map((item) => item.trim()),
    methods: (_d = process.env.CORS_ALLOW_METHODS) === null || _d === void 0 ? void 0 : _d.split(",").map((item) => item.trim()),
    preflightContinue: ["true", "1"].includes(process.env.CORS_PREFLIGHT_CONTINUE || ""),
};
...
const InjectEnv = (object) => {
    for (const Key in object) {
        ...
        if (typeof Value === "string") {
            ...
            const MatchObject = /(object|boolean|number|string|list)\(\s*\{\s*\{\s*(\w+)\s*\}\s*\}\s*\)/.exec(Value);
            if (MatchObject) {
                if (MatchObject[1] === "object")
                    object[Key] = JSON.parse(process.env[MatchObject[2]] || "{}");
                else if (MatchObject[1] === "boolean")
                    object[Key] = ["1", "true"].includes((process.env[MatchObject[2]] || "").toString().toLowerCase());
                    ...
                    object[Key] = Value.replace(/\{\s*\{\s*(\w+)\s*\}\s*\}/g, (_, key) => process.env[key]);
            }
            else
                object[Key] = Value.replace(/\{\s*\{\s*(\w+)\s*\}\s*\}/g, (_, key) => process.env[key]);
        }
